...
package chessflash.view {
	...
	public class BtnRewind extends BtnBase {
		
/// Constructor
		
		public function BtnRewind(width:uint, height:uint, lm:LayoutManager, up:Boolean, name:String) {
...
/// Override Protected Functions			
		...
		override protected function createUpState(): Sprite {
			...
			var image1:Shape = createRoundedTriangleLeft(_foreground, _alphaUp);
			var image2:Shape = createRoundedLeftBar(_foreground, _alphaUp);
		...
		override protected function createOverState(down:Boolean): Sprite {
			var sprite:Sprite = new Sprite();
			var background:Shape = createBackground();
			var image1:Shape = createRoundedTriangleLeft(_foreground, _alphaOver);
			var image2:Shape = createRoundedLeftBar(_foreground, _alphaOver);
...
/// Private Functions		
		...
		private function createRoundedTriangleLeft(color:uint, alpha:Number): Shape {
			...
			triangle.graphics.lineTo(0 + _lineStyleWidth, _height / 2);
		...
		private function createRoundedLeftBar(color:uint, alpha:Number): Shape {
			var bar:Shape = new Shape();
			bar.graphics.lineStyle (_lineStyleWidth, _foreground);
			bar.graphics.beginFill (color, alpha);
			bar.graphics.moveTo(0, _height);
